...
#endif
    Core::Get<Analytics>()->AddErrorEvent(Analytics::ErrorSeverity::Warning, String::Format(TEXT("{0} in file {1} at line {2}"), message, String(file), line).ToStringAnsi().GetText());
}
...
#endif
    Core::Get<Analytics>()->AddErrorEvent(Analytics::ErrorSeverity::Error, String::Format(TEXT("{0} in file {1} at line {2}"), message, String(file), line).ToStringAnsi().GetText());
}
...
void Logger::Critical(bool shutdown, const StringView& message, const char* file, int line)
{
    ...
    DebugLog::LogError(message);
#endif
    Core::Get<Analytics>()->AddErrorEvent(Analytics::ErrorSeverity::Critical, String::Format(TEXT("{0} in file {1} at line {2}"), message, String(file), line).ToStringAnsi().GetText());
    if (shutdown)
    {
